...
Use this mode to put an exact existing AAB version on one or more existing testing tracks at full rollout. It does not need bundletool or a new upload. The source can be production, but production destinations are rejected.
...
This mode is covered by mocked lifecycle and transport tests, not yet a live publishing validation. Keep that limitation explicit until an authorized test release completes.
